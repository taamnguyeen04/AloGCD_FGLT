...
def init_experiment(args, runner_name=None, exp_id=None):
    ...
    if getattr(args, 'use_exact_exp_root', False):
        log_dir = os.path.join(args.exp_root, 'logs')
        tensorboard_dir = os.path.join(args.exp_root, 'tensorboard')
        model_root_dir = os.path.join(args.exp_root, 'checkpoints')
    else:
        ...
        model_root_dir = os.path.join(log_dir, 'checkpoints')
    ...
    args.writer = SummaryWriter(log_dir=tensorboard_dir)
